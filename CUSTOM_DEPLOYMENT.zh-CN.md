...
1. 找到“参数覆盖”。
2. 直接点击 `GPT-5.6 SOL` 或 `Claude Fable 5` 快捷按钮。
3. 如需调整，点击“可视化编辑”检查或追加规则。
4. 保存渠道后逐个执行渠道测试。

快捷按钮只更新当前表单，不会在点击时立即修改数据库，也不会自动覆盖其他渠道。可视化编辑器的模板下拉框中也提供同样的两套预设。

渠道新增/编辑抽屉的“路由策略”还支持按渠道设置并发、RPM 和 TPM 上限，`0` 表示不限制。达到任一上限的渠道会被立即跳过，请求自动分流到其他匹配渠道；只有所有匹配渠道都达到容量上限时才返回限流错误，不会在已满渠道上排队。

以下 SQL 脚本仅作为已知生产渠道的批量应用方案保留，不再是单渠道配置的首选方式。
